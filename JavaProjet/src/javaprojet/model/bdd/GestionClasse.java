...
public class GestionClasse {
    ...
    public static void updateClasse(Classe c){
        try {
            DBconnexion.connexionDB();
            Statement stmt = DBconnexion.getConn().createStatement();
            String update = "UPDATE `classe` SET `NomClasse` = '" + c.getNomClasse() + "',`idMatiere1` = '" + c.getIdMatiere1() + "', `idMatiere2` = '" + c.getIdMatiere2() + "', `idMatiere3` = '" + c.getIdMatiere3() + "', `idMatiere4` = '" + c.getIdMatiere4() + "' WHERE `classe`.`idClasse` ='" + c.getIdClasse() + "' ;";   		stmt.executeUpdate(update);
            JOptionPane jop1 = new JOptionPane();
            JOptionPane.showMessageDialog(null, "Classe mis à jour dans la base de donnée", "Ajout BDD", JOptionPane.INFORMATION_MESSAGE);
		
            System.out.println("Classe modifie ");
	} catch (SQLException SQLe) {
            System.out.println("Echec de la mise à jour dans la base : "+SQLe.getMessage());
            JOptionPane jop1 = new JOptionPane();
            JOptionPane.showMessageDialog(null, SQLe.getMessage(), "Erreur BDD", JOptionPane.ERROR_MESSAGE);
	}     
    }  
}
